...
let serialConnectWeb = async () => {
  ...
  try {
    ...
    if (e.toString().indexOf("Failed to read the 'baudRate' property") >= 0) {
      ...
      write(chunk) {
        if (!firmwareUpdateMode) {
          ...
          if (serialLastData.length > 300) {
            serialLastData = serialLastData.substring(
              serialLastData.length - 300,
              serialLastData.length
            );
          }
          if (dashboardIsReady) {
...
let showPortSelect = () => {
  return new Promise(async (resolve, reject) => {
    ...
    for (let port of await serialAPI.list()) {
      $('#port-list').append(
        `<li data-port="${port.path}"><i class="fab fa-usb"></i> ${port.path} - ${port.manufacturer}</li>`
      );
    }
...
let boardReset = (enterToBootMode) => {
  // Hard-reset
...
class UploadOnBoot {
  ...
  async getFirmwareInfo() {
    ...
    let checkVersion =
      /MicroPython\s+([^\s]+)\s+on\s+([0-9\-]+);\s?(.+)\s+with\s+([^\s]+)$/m.exec(
        serialLastData
      );
    if (!checkVersion) {
...
class UploadViaREPL {
  ...
  async start() {
    ...
    serialLastData = '';
    if (!(await this.sendByteLoopWaitNextCommand(3, 100, 100))) {
      // Ctrl + C
    ...
    RawREPLMode = true;
    if (!(await this.sendByteLoopWaitNextCommand(1, 50, 100))) {
      // Ctrl + A, Enter to Raw REPL
  ...
  async upload(fileName, content) {
    ...
    for (const chunkContent1 of chunkContent1Array) {
      serialLastData = '';
      if (
        !(await this.sendLineLoopWaitMatch(
          `f = open("${fileName}", "${firstWriteFlag ? 'w' : 'a'}");w=f.write;p=print`,
          /OK[^>]*>$/gm,
          isElectron ? 50 : 100,
          20
        ))
      ...
      if (chipId === 'ESP32') {
        for (const chunkContent2 of chunkContent1.match(/.{1,100}/gs)) {
          ...
          if (
            !(await this.sendLineLoopWaitMatch(
              `p(w(${JSON.stringify(chunkContent2).replace(
                /[\u007F-\uFFFF]/g,
        ...
        if (
          !(await this.sendLineLoopWaitMatch(
            `p(w(${JSON.stringify(chunkContent1).replace(
              /[\u007F-\uFFFF]/g,
              (chr) => '\\u' + ('0000' + chr.charCodeAt(0).toString(16)).substr(-4)
            )}))`,
            /OK[0-9]{1,3}[^>]*>/gm,
  ...
  if (uploadModuleList.length > 0) {
    ...
    if (isElectron) {
      ...
      for (const extensionId of nodeFS.ls(extensionDir)) {
        for (const filePath of await nodeFS.walk(
          `${extensionDir}/${extensionId}/modules`
        )) {
...
let findIncludeModuleNameInCode = (code) => {
  const regex = /^\s*?(?:import|from)\s+([^\s]+)/gm;
